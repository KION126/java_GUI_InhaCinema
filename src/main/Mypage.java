...
public class Mypage extends javax.swing.JFrame {
    ...
    private void btnBackActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBackActionPerformed
        // 뒤로가기 버튼
        MainFrame frame = new MainFrame(user);  // 메인페이지로 전환
    ...
    private void btnReserveInfoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReserveInfoActionPerformed
        // 예매정보 버튼
        CardLayout card = (CardLayout)pnMain.getLayout();   // 예매정보 페이지로 전환
        card.show(pnMain, "pnReserveInfo");
    }//GEN-LAST:event_btnReserveInfoActionPerformed

    private void btnReserveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnReserveActionPerformed
        // 예매내역
        btnReserve.setEnabled(false);   // 예매내역 페이지로 전환
    ...
    private void btnCancelListActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCancelListActionPerformed
        // 취소내역
        btnCancelList.setEnabled(false);    // 예매취소내역 페이지로 전환
    ...
    private void tbReserveInfoMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbReserveInfoMouseClicked
        ...
        if (evt.getClickCount() == 2) {     // 더블클릭 수행 시
            ...
            if (row >= 0 && col >= 0) {     // 유효성 검사
                CardLayout card = (CardLayout)pnMain.getLayout();   // 예매 상세 페이지로 전환
    ...
    private void btnMyInfoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMyInfoActionPerformed
        // 내 정보 버튼
        CardLayout card = (CardLayout)pnMain.getLayout();   // 내 정보 페이지로 전환
        card.show(pnMain, "pnMypage");
    }//GEN-LAST:event_btnMyInfoActionPerformed

    private void btnCancelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCancelActionPerformed
        // 예매 취소 버튼
        int userChoice = JOptionPane.showConfirmDialog(     // 예매 취소 재확인
                null,
                "정말로 예매를 취소하시겠습니까?",
                "확인",
    ...
    private void getReservationInfo(){  // 예매 내역 테이블을 정의하는 메서드
        DB db = new DB();
        ...
        try {                                                                                  // 예매 취소가 안되어 있는 예매 정보 추출
            ...
            while(db.rs.next()) {
                DefaultTableModel rowData = (DefaultTableModel) tbReserveInfo.getModel();       // 테이블을 재정의 할 객체 생성
                ...
                rowData.addRow(new Object[]{reservationDate, movieTitle, screeningDate, screeningTime});    // 테이블에 객체 삽입
            }
    ...
    private void getCancelInfo(){   // 예매 취소 내역 테이블을 정의하는 메서드
        DB db = new DB();
        String sql = "SELECT r.groupID, MIN(r.date) AS ReservationDate, m.title AS MovieTitle, " +
        ...
        try {                                                                                  // 예매 취소가 되어 있는 예매 정보 추출
            ...
            while(db.rs.next()) {
                DefaultTableModel rowData = (DefaultTableModel) tbCancelInfo.getModel();    // 테이블을 재정의 할 객체 생성
                
                String reservationDate = db.rs.getString("ReservationDate"); // 예매 날짜
                String movieTitle = db.rs.getString("MovieTitle");           // 영화 제목
                String cancelTime = db.rs.getString("CancelTime");           // 취소 날짜, 시각
                
                rowData.addRow(new Object[]{reservationDate, movieTitle, cancelTime});  // 테이블에 객체 삽입
            }
        } catch(SQLException e) {
            System.out.println("getSeat SQLException : " + e.getMessage());
        } finally {
    ...
    private String getGroupID(String date){ // 선택된 예매 정보의 티켓 그룹ID 추출
        DB db = new DB();
    ...
    private void getTiketInfo(String date){ // 해당 티켓 정보 추출
        DB db = new DB();
    ...
    private void cancelTiket(String groupID){   // 예매 취소 메서드
        DB db = new DB();
        String sql = "UPDATE inhaCinema.reservation SET canceled = 1, cancelTime = NOW() WHERE groupID = ?";    // canceled와 cancelTime update
